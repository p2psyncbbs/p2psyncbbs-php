...
$_POST['FROM'] = htmlspecialchars($_POST['FROM'], ENT_COMPAT | ENT_HTML401, 'SJIS');
...
elseif ($_POST['key']) {
  ...
  fclose($fp);
  #レス数がTHREAD_RESを超えていれば書けない
  if ($imgnum > THREAD_RES) DispError("ＥＲＲＯＲ！","ＥＲＲＯＲ：このスレッドには書けません！");
...
#====================================================
#ＰＯＳＴ情報
if ( !preg_match( '/書き込む|かきこむ|新規スレッド作成/', $_POST['submit'])) {
...
if ( $SETTING['BBS_NO_ID'] == 'checked') $ID = 'ID:???';
# ホスト表示の場合ホスト名を付加
if ($SETTING['BBS_DISP_IP'] == "checked") $ID = "HOST:$HOST";
...
#====================================================
#　レスポンスアンカー（本文）#2013年09月25日(水) アンカーをhtmlリンクに変換しない。する必要がない。
#====================================================
...
require 'p2p_make_work.php';
$subtt = MakeWorkFile( $_POST, $DATE, $ID, $NOWTIME);
#====================================================
#　ファイル操作（subject.txt）
#====================================================
